...
angular.module("umbraco").controller("Umbraco.Dialogs.LoginController",
    function ($scope, $cookies, localizationService, userService, externalLoginInfo) {
        ...
        $scope.Submitgooglefactor = function () {
            
            var otpval = angular.element('#otp').val()
            console.log($scope.googleotpkey);
            userService.google2factorvaidate($scope.googleotpkey, otpval).then(function (result) {

                console.log(result);
                userService.authenticatenredirecttodashboard($scope.loginname, $scope.loginpassword).then(function (data) {
                    $scope.submit(true);
                });
        ...
        $scope.loginSubmit = function (login, password) {
            ...
            userService.authenticate(login, password)
                .then(function (data) {
                    ...
                    //$scope.loginOTPcode = true;
                    if (data != null) {

                        userService.google2factor(data.user.email).then(function (data) {

                            //console.log(data);
                            $scope.loginname = login;
                            $scope.loginpassword = password;
                            $scope.loginOTPcode = true;
                            $scope.googleotpimageUrl = data["data"]["QrCodeSetupImageUrl"];
                            $scope.googleotpkey = data["data"]["AccountSecretKey"];
                            console.log($scope.googleotpimageUrl);
                        });
